...
class ConstantEditorWidget(QWidget):
    ...
    def populateConstantsTable(self):
        ...
        for constantName, constant in self.database.constants.items():
            if constantName not in BalloonPackageDatabase.AUTOGENERATED_CONSTANT_NAMES:
    ...
    def changingValue(self, valueData):
        ...
        print(value)
        self.database.updateConstant(dataclasses.replace(constant, value=value), oldName=constant.name)
        self.change.emit()
    ...
    def changingConstant(self, row, col, text):
        constantName = list(self.database.constants.keys())[row]
        if col == 0:
            newConstant = dataclasses.replace(self.database.constants[constantName], name=text)
        elif col == 3:
            newConstant = dataclasses.replace(self.database.constants[constantName], description=text)
        else:
            raise RuntimeError(f'Invalid column {col}')
        self.database.updateConstant(newConstant, oldName=constantName)
        self.change.emit()
